...
try{
  configs = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'fife.json'), {encode: 'utf8'}).toString());
}catch(e){
  if(['livereload', 'runserver'].indexOf(process.argv[2]) > -1){
    utils.console.log(utils.console.red('当前目录找不到配置文件 fife.json 或文件格式错误, 请先执行 fife generate!'));
    throw e;
  }
  configs = {};
}
